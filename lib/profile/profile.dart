...
class UnlockedList extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    ...
    Stream<QuerySnapshot> _places = db
        .collection('places')
        .orderBy('city')
        .snapshots();
        ...
        builder: (context, snapshot) {
          return StreamBuilder<QuerySnapshot>(
              ...
              builder: (context, snapshot2) {
                ...
                return MaterialApp(
                  title: 'My Unlocked Places',
                  home: Scaffold(
                    appBar: AppBar(
                      title: Text('My Unlocked Places'),
                    ),
                    body: ListView(
                        children: _buildTiles(snapshot2.data!.docs)
                    ),
                  ),
                );
              });
  ...
  List<Widget> _buildTiles(List<DocumentSnapshot> unlocked) {
    var tiles = <Widget>[];
    var places = <DocumentSnapshot>[];

    for(var ulk in unlocked) {
      db.collection('places').doc(ulk.id).get().then((snapshot) => places.add(snapshot));
    }
    for(var p in places) {
      tiles.add(ListTile(
        title: Text(p.get('name').toString()),
      ));
    }
    return tiles;
  }
